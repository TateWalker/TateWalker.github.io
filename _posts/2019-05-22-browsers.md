...
A lot of people out there think that a [search engine]({{site.url}}/search-engines) is the same as an internet browser. This is most likely due to the fact that Google has **major** products in both fields, so they become synonymous. However, these two things are vastly different.
...
If you want a simple fix [similar to how we switched our search engine]({{site.url}}/search-engines) to DuckDuckGo, you'll need to switch your browser to either Safari or Firefox.
